...
export const Testimonials = () => {
    ...
    return (
        <section id="depoimentos" className="py-16 px-4 bg-gradient-to-b from-slate-50 to-white">
        <div className="container mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-600 max-w-2xl mx-auto leading-relaxed font-light">
              Histórias reais de transformação e cura através da Constelação Familiar
            </p>
          </div>
              ...
              >
                <div className="flex justify-center mb-4">
                  {[...Array(testimonial.rating)].map((_, index) => (
                    ...
                    <Star key={index} className="w-4 h-4 text-orange-500 fill-current" />
                  ))}
                </div>
                <blockquote className="text-slate-600 leading-relaxed font-light text-center mb-4 italic">
                  "{testimonial.text}"
                </blockquote>
                <p className="text-slate-800 font-light text-center tracking-wide text-sm">— {testimonial.name}</p>
              </div>
            ))}
          </div>
        </div>
